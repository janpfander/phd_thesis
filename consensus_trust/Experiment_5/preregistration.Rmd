...
data <- tibble(
  ...
  independence = rep(c("independent", "conflict"), each = 50),
...
```{r table, echo=FALSE}
# Create a matrix of image file paths as Markdown-formatted strings
table <- data.frame(independence = "The three advisors are independent of each other, and have no conflict of interest in making investment recommendations.",
                          conflict = "The three advisors have already invested in one of the three options, the same option for all three. As a result, they have an incentive to push that option in their recommendations.")
# Use kable() to create the table and print it as Markdown
kableExtra::kable(table, format = "markdown",
                  col.names = c("Independent", "Biased (conflict of interest)"), 
                  align = "l",
                  caption = "Participants read only one of these introductions before seeing the sets of estimates, depending on which condition they had been assigned to")
```

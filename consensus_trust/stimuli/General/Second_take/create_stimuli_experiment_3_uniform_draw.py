...
def create_stimuli_experiment_uniform_draw(
    ...
    for sample in range(n_samples):
        data_dir = os.path.join(
            os.getcwd(),
            f"experiment_3_globalseed_{global_seed}_sample{sample+1}_npoints_{list_npoints}_span_{list_span}_highlight_{highlight_arrow}_distrib_{distrib_type}",
        )
        ...
        mean_counter = 0
        list_mean_generator = np.random.default_rng(list_seeds[seed_counter])
        list_mean_generator.shuffle(list_mean)
